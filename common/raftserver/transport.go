...
func (tr *transport) AddMember(m Member) {
	tr.mu.Lock()
	if _, hit := tr.senders[m.Id]; !hit {
		tr.senders[m.Id] = newTransportSender(m.Id, m.Host)
	}
	tr.mu.Unlock()
}
...
func (tr *transport) SetMembers(members []Member) {
	...
	for _, m := range members {
		if sender, hit := tr.senders[m.Id]; hit {
			senderMap[m.Id] = sender
		} else {
			senderMap[m.Id] = newTransportSender(m.Id, m.Host)
		}
	}
...
func (tr *transport) handleSnapshot(w http.ResponseWriter, r *http.Request) {
	...
	snap.(*applySnapshot).meta = sm
	log.Infof("recv snapshot(%s) meta: %s", sm.Name, sm.Meta.String())
