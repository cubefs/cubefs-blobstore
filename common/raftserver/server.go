...
type RaftServer interface {
	...
	AddMember(ctx context.Context, peerId uint64, host string) error
	AddLearner(ctx context.Context, peerId uint64, host string) error
	RemoveMember(ctx context.Context, peerId uint64) error
	IsLeader() bool
	Status() Status
...
func (s *raftServer) AddMember(ctx context.Context, peerId uint64, host string) (err error) {
	id := s.idGen.Next()
	cc := pb.ConfChange{
		ID:      id,
...
func (s *raftServer) AddLearner(ctx context.Context, peerId uint64, host string) (err error) {
	id := s.idGen.Next()
	cc := pb.ConfChange{
		ID:      id,
...
func (s *raftServer) applyConfChange(entry pb.Entry) {
	...
	if err := cc.Unmarshal(entry.Data); err != nil {
		log.Errorf("unmarshal confchange error: %v", err)
		return
	}
	...
	case pb.ConfChangeAddNode:
		s.addMember(cc.NodeID, string(cc.Context), false)
...
func (s *raftServer) applySnapshot(snap Snapshot) {
	sm := snap.(*applySnapshot).meta
	nr := snap.(*applySnapshot).nr
	log.Infof("apply snapshot(%s) data......", sm.Name)
	// read snapshot data
	if err := s.sm.ApplySnapshot(SnapshotMeta(sm.Meta), snap); err != nil {
		log.Errorf("apply snapshot(%s) error: %v", sm.Name, err)
		nr.notify(err)
		return
	}
...
func (s *raftServer) addMember(id uint64, host string, learner bool) {
	m := Member{
		Id:      id,
		Host:    host,
		Learner: learner,
