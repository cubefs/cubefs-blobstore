...
var cfgs = [3]*Config{
	{
		...
		TickInterval: 1,
		ElectionTick: 3,
		Peers:        map[uint64]string{1: "127.0.0.1:9090", 2: "127.0.0.1:9091", 3: "127.0.0.1:9092"},
	},
	{
		NodeId:       2,
		ListenPort:   9091,
...
func TestRaftServer(t *testing.T) {
	...
	mbs := cfgs[prevIdx].GetMembers()
	require.Equal(t, 3, len(mbs.Mbs))
	rss[prevIdx], err = NewRaftServer(cfgs[prevIdx])
	require.Nil(t, err)

	nodeId := cfgs[int(prevIdx)].NodeId
	host := cfgs[int(prevIdx)].Peers[nodeId]
	log.Infof("add member(id:%d, host:%s)", nodeId, host)
	err = rss[idx].AddMember(context.TODO(), nodeId, host)
	require.Nil(t, err)
	...
	learnerCfg := Config{
		...
		ElectionTick: 3,
		Peers:        map[uint64]string{1: "127.0.0.1:9090", 2: "127.0.0.1:9091", 3: "127.0.0.1:9092"},
		Learners:     map[uint64]string{4: "127.0.0.1:9093"},
	}
	...
	err = rss[0].AddMember(context.TODO(), 4, "127.0.0.1:9093")
	require.Nil(t, err)
	status = rss[int(learnerSM.leader)-1].Status()
	require.Equal(t, 4, len(status.Peers))
