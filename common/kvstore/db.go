...
func OpenDB(path string, isSync bool, opt *RocksDBOption) (KVStore, error) {
	if path == "" {
...
func OpenDBWithCF(path string, isSync bool, opt *RocksDBOption, cfnames []string) (KVStore, error) {
	if path == "" {
		return nil, &os.PathError{Op: "open", Path: path, Err: syscall.ENOENT}
	}
