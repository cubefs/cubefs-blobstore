...
func fixWithDefaultRocksDBOption(opt RocksDBOption) RocksDBOption {
	...
	if !opt.ErrorIfExists {
		opt.ErrorIfExists = def.ErrorIfExists
	}
	if opt.LRUCache == 0 {
	...
	if !opt.EnableStatistics {
		opt.EnableStatistics = def.EnableStatistics
	}
	if !opt.AllowMMAPRead {
		opt.AllowMMAPRead = def.AllowMMAPRead
	}
	if !opt.AllowMMAPWrite {
		opt.AllowMMAPWrite = def.AllowMMAPWrite
	}
	if !opt.InplaceUpdateSupport {
		opt.InplaceUpdateSupport = def.InplaceUpdateSupport
	}
