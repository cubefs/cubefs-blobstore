...
func CheckVunitLocations(locations []VunitLocation) bool {
	...
	for _, location := range locations {
		...
		if location.DiskID == InvalidDiskID {
			return false
		}
	}
	return true
...
const (
	MigrateStateInited MigrateSate = iota + 1
...
type MigrateTask struct {
	...
	State         MigrateSate `json:"state" bson:"state"`                     // task state
...
type SvrInfo struct {
	...
	ClusterID ClusterID `json:"cluster_id" bson:"cluster_id"`
