...
func GetBenchmarkBids(
	...
	globalReplicas := base.AbstractGlobalStripeReplicas(replicas, mode, badIdxs)
	...
	for _, bid := range allBidsList {
		markDel := false
		existStatus := base.NewBidExistStatus(mode)
		notExistCnt := 0
		...
		if markDel {
			base.DroppedBidRecorderInst().Write(
				ctx,
				replicas[0].Vuid.Vid(),
				bid.Bid,
				"mark deleted",
		...
		if notExistCnt > base.AllowFailCnt(mode) {
			base.DroppedBidRecorderInst().Write(
				ctx,
				replicas[0].Vuid.Vid(),
				bid.Bid,
...
func minWellReplicasCnt(mode codemode.CodeMode) int {
	return base.ModeN(mode) + base.AllowFailCnt(mode)
}
