...
func TestShardsBuf(t *testing.T) {
	bp := base.NewByteBufferPool(1024*1024, 20)
...
func TestMemNotEnough(t *testing.T) {
	...
	repair.bufPool = base.NewByteBufferPool(16*1024*1024, 1)
	badi := []uint8{0, 1, 2, 3, 4, 5}
	err := repair.RecoverShards(ctx, badi, true)
	require.Error(t, err)
	require.EqualError(t, base.ErrMemNotEnough, err.Error())

	// local stripe
	repair1, repairBids, _, _ := InitMockRepair(codemode.EC6P3L3)
	repair1.bufPool = base.NewByteBufferPool(16*1024*1024, 1)
	badi1 := []uint8{11}
	...
	repair1.bufPool = base.NewByteBufferPool(16*1024*1024, 4)
	badi1 = []uint8{11}
	...
	repair2.bufPool = base.NewByteBufferPool(16*1024*1024, 1)
	badi2 := []uint8{0}
	...
	repair2.bufPool = base.NewByteBufferPool(16*1024*1024, 4)
