...
// LoopExecUntilSuccess execute f until success
func LoopExecUntilSuccess(ctx context.Context, msg string, f func() error) {
	span := trace.SpanFromContextSafe(ctx)
	for {
		err := f()
		if err == nil {
		...
		span.Errorf("%s execute fail err: %+v", msg, err)
