...
import (
	...
	"github.com/cubefs/blobstore/common/memcache"
		...
		for {
			...
			for dv := range cache {
				go func(dv discardVid) {
					h.tryDiscardVidOnAllocator(dv.cid, dv.vid, allocator.AllocVolsArgs{
						Fsize:    1,
						BidCount: 1,
...
func (h *Handler) tryDiscardVidOnAllocator(cid proto.ClusterID, vid proto.Vid, args allocator.AllocVolsArgs) {
	...
	_, err = h.allocatorClient.VolumeAlloc(ctx, host, &args)
	if err != nil {
