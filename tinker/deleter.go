...
const (
	DefaultRetryIntervalMs = 3000
	DefaultDelDelay        = 10 * time.Minute
)
...
type BlobDeleteConfig struct {
	...
	NormalTopic              base.KafkaConfig  `json:"normal_topic"`
	...
	DelLog         recordlog.Config `json:"dellog"`
}

// DeleteMgr is blob delete manager
type DeleteMgr struct {
...
// NewDeleteMgr returns blob delete manager
func NewDeleteMgr(
	...
	if cfg.SafeDelayTimeH == 0 {
		// reduce the probability of shard repair and delete concurrency by delete blob delay a duration
		safeDelayTime = DefaultDelDelay
	} else if cfg.SafeDelayTimeH < 0 {
		safeDelayTime = time.Hour * time.Duration(0)
	} else {
		safeDelayTime = time.Hour * time.Duration(cfg.SafeDelayTimeH)
	}

	normalTopicConsumers, err := base.NewKafkaPartitionConsumers(&cfg.NormalTopic, offAccessor)
	if err != nil {
		return nil, err
	}
	...
	if cfg.FailMsgConsumeIntervalMs == 0 {
		cfg.FailMsgConsumeIntervalMs = DefaultRetryIntervalMs
	}

	delLogger, err := recordlog.NewEncoder(&cfg.DelLog)
	if err != nil {
		return nil, err
	}
...
func (d *deleteTopicConsumer) deleteWithCheckVolConsistency(ctx context.Context, vid proto.Vid, bid proto.BlobID) error {
	return DoWithCheckVolConsistency(ctx, d.volCache, vid, func(info *client.VolInfo) error {
		return d.deleteBlob(ctx, info, bid)
	})
}

func (d *deleteTopicConsumer) deleteBlob(ctx context.Context, volInfo *client.VolInfo, bid proto.BlobID) (err error) {
...
func (d *deleteTopicConsumer) markDelBlob(ctx context.Context, volInfo *client.VolInfo, bid proto.BlobID) (*client.VolInfo, error) {
	return d.deleteShards(ctx, volInfo, bid, true)
}

func (d *deleteTopicConsumer) delBlob(ctx context.Context, volInfo *client.VolInfo, bid proto.BlobID) (*client.VolInfo, error) {
...
func (d *deleteTopicConsumer) deleteShards(
	...
	bid proto.BlobID,
	markDelete bool) (new *client.VolInfo, err error) {
	span := trace.SpanFromContextSafe(ctx)
