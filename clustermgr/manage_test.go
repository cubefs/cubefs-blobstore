...
func TestManage(t *testing.T) {
	...
	// test snapshot dump
	{
		...
		normalDB, err := normaldb.OpenNormalDB(tmpNormalDBPath, false, nil)
		assert.NoError(t, err)
		defer normalDB.Close()
		volumeDB, err := volumedb.Open(tmpVolumeDBPath, false, nil)
		assert.NoError(t, err)
		defer volumeDB.Close()
		raftDB, err := raftdb.OpenRaftDB(tmpRaftDBPath, false, nil)
		assert.NoError(t, err)
		defer raftDB.Close()
