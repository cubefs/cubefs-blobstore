...
func TestSnapshot(t *testing.T) {
	...
	normalDB1, err := normaldb.OpenNormalDB(tmpNormalDBPat1, false, &kvstore.RocksDBOption{ReadOnly: false})
	assert.NoError(t, err)
	defer normalDB1.Close()

	normalDB2, err := normaldb.OpenNormalDB(tmpNormalDBPat2, false, &kvstore.RocksDBOption{ReadOnly: false})
	assert.NoError(t, err)
	...
	// create snapshot and apply snapshot
	{
		...
		raftDB, err := raftdb.OpenRaftDB(tmpDBPath, false, &kvstore.RocksDBOption{ReadOnly: false})
		assert.NoError(t, err)
		defer raftDB.Close()
		raftNode, err := NewRaftNode(&RaftNodeConfig{FlushNumInterval: 1, TruncateNumInterval: 1, ApplyIndex: 1}, raftDB)
		assert.NoError(t, err)

		dbName1 := "normal1"
		dbName2 := "normal2"

		snapshot := raftNode.CreateRaftSnapshot(map[string]SnapshotDB{dbName1: normalDB1, dbName2: normalDB2}, 10)
		...
		applyNormalDB1, err := normaldb.OpenNormalDB(applyNormalDBPat1, false, &kvstore.RocksDBOption{ReadOnly: false})
		assert.NoError(t, err)
		defer applyNormalDB1.Close()

		applyNormalDB2, err := normaldb.OpenNormalDB(applyNormalDBPat2, false, &kvstore.RocksDBOption{ReadOnly: false})
		assert.NoError(t, err)
		defer applyNormalDB2.Close()

		err = raftNode.ApplyRaftSnapshot(ctx, map[string]SnapshotDB{dbName1: applyNormalDB1, dbName2: applyNormalDB2}, snapshot)
		assert.NoError(t, err)
