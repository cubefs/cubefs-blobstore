...
func initServiceWithData() *Service {
	...
	generateVolume(cfg.VolumeMgrConfig.VolumeDBPath, cfg.NormalDBPath)
...
func generateVolume(volumeDBPath, NormalDBPath string) {
	volumeDB, err := volumedb.Open(volumeDBPath, false, &kvstore.RocksDBOption{ReadOnly: false})
	if err != nil {
		log.Error("open db error")
	}
	normalDB, err := normaldb.OpenNormalDB(NormalDBPath, false, &kvstore.RocksDBOption{ReadOnly: false})
	if err != nil {
		log.Error("open db error")
	}
