...
func newRocksDB(path string, conf MetaConfig) (db rdb.KVStore, err error) {
	...
	db, err = rdb.OpenDB(path, conf.Sync, &conf.RocksdbOption)
	if err != nil {
		return
	}
