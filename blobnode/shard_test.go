...
func TestGetShards(t *testing.T) {
	service, _ := newTestBlobNodeService(t, "GetShards")
	defer cleanTestBlobNodeService(service)
	...
	url := fmt.Sprintf("%v/shards", host)
	// /shards
	arg := &bnapi.GetShardsArgs{
		DiskID: diskID,
		Vuid:   vuid,
		Bids:   []proto.BlobID{8},
	}
	...
	require.NoError(t, err)
	request, err := http.NewRequest(http.MethodGet, url, bytes.NewReader(b))
	...
	arg = &bnapi.GetShardsArgs{
		DiskID: diskID,
		Vuid:   vuid,
		Bids:   make([]proto.BlobID, BidBatchReadLimit+1),
	}
	b, err = json.Marshal(arg)
	require.NoError(t, err)
	request, err = http.NewRequest(http.MethodGet, url, bytes.NewReader(b))
	...
	_, err = client.GetShards(ctx, host, &bnapi.GetShardsArgs{
		Vuid: vuid,
		Bids: []proto.BlobID{},
	})
	require.Error(t, err)
	...
	_, err = client.GetShards(ctx, host, &bnapi.GetShardsArgs{
		Vuid: vuid,
		Bids: []proto.BlobID{proto.InValidBlobID},
	})
	require.Error(t, err)
	...
	sis, _, err := client.ListShards(ctx, host, listChunkArg)
	require.NoError(t, err)
	require.Equal(t, 10, len(sis))

	bids := make([]proto.BlobID, 0)
	for i := range sis {
		bids = append(bids, sis[i].Bid)
	}
	getShardsArg := &bnapi.GetShardsArgs{
		DiskID: diskID,
		Vuid:   vuid,
		Bids:   bids,
	}
	body, err := client.GetShards(ctx, host, getShardsArg)
	require.NoError(t, err)
	defer body.Close()

	shardsData, err := ioutil.ReadAll(body)
	require.NoError(t, err)
	require.Equal(t, len(bids)*shardSize, len(shardsData))
	for i := range bids {
		require.Equal(t, true, reflect.DeepEqual(shardData[:], shardsData[i*shardSize:(i+1)*shardSize]))
	}
}
